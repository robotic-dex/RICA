...
static Texture2D FontTexture = { 0 }; // Текстура для шрифтов
...
{
    // FIX: Используем BackendPlatformUserData для хранения данных
    return ImGui::GetCurrentContext() ? static_cast<ImGui_ImplRaylib_Data*>(ImGui::GetIO().BackendPlatformUserData) : nullptr;
}
...
void ImGui_ImplRaylib_CreateBackendData()
{
    ...
    ImGui::GetIO().BackendPlatformUserData = MemAlloc(sizeof(ImGui_ImplRaylib_Data));
}
...
void ImGui_ImplRaylib_FreeBackendData()
{
    ...
    MemFree(ImGui::GetIO().BackendPlatformUserData);
    ImGui::GetIO().BackendPlatformUserData = nullptr;
}

// FIX: Функция для принудительной загрузки шрифтов (решает проблему Assertion Failed)
void rlImGuiReloadFonts()
{
    ...
    io.Fonts->SetTexID((ImTextureID)(uintptr_t)FontTexture.id);
}
...
static void ImGuiNewFrame(float deltaTime)
{
    ...
    // FIX: Проверка загрузки шрифтов
    if (FontTexture.id == 0) rlImGuiReloadFonts();
...
static void ImGuiRenderTriangles(unsigned int count, int indexStart, const ImVector<ImDrawIdx>& indexBuffer, const ImVector<ImDrawVert>& vertBuffer, ImTextureID texturePtr)
{
    ...
    // FIX: Приведение типов для 64-bit систем
...
void SetupBackend(void)
{
    ...
    // FIX: Настройка колбэков через io
...
void rlImGuiEndInitImGui(void)
{
    ...
    // FIX: Принудительная загрузка шрифтов
    rlImGuiReloadFonts();
}
...
bool rlImGuiImageButton(const char* name, const Texture* image)
{
    ...
    return ImGui::ImageButton(name, (ImTextureID)(uintptr_t)image->id, ImVec2(float(image->width), float(image->height)));
}
...
bool rlImGuiImageButtonSize(const char* name, const Texture* image, Vector2 size)
{
    ...
    return ImGui::ImageButton(name, (ImTextureID)(uintptr_t)image->id, ImVec2(size.x, size.y));
}
...
void rlImGuiImageSize(const Texture* image, int width, int height)
{
    ...
    ImGui::Image((ImTextureID)(uintptr_t)image->id, ImVec2(float(width), float(height)));
}
...
void rlImGuiImageSizeV(const Texture* image, Vector2 size)
{
    ...
    ImGui::Image((ImTextureID)(uintptr_t)image->id, ImVec2(size.x, size.y));
}
...
void rlImGuiImageRect(const Texture* image, int destWidth, int destHeight, Rectangle sourceRect)
{
    ...
    ImGui::Image((ImTextureID)(uintptr_t)image->id, ImVec2(float(destWidth), float(destHeight)), uv0, uv1);
}
...
// FIX: Комментируем этот блок, так как ImTextureData отсутствует в 1.90.4
//void ImGui_ImplRaylib_UpdateTexture(ImTextureData* tex)
//{
    // ... code removed for compatibility ...
//}

void ImGui_ImplRaylib_RenderDrawData(ImDrawData* draw_data)
{
    // FIX: Убираем обработку draw_data->Textures (experimental feature)
    /*
