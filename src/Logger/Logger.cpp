...
bool Log::addLog(LogLevel type, std::string module, std::string msg, std::string path)
{
    // текущее время
    ...
    // логирование
    file << "[" << local_time->tm_year + 1900 << "-"
         << local_time->tm_mon + 1 << "-"
         << local_time->tm_mday << " "
         << local_time->tm_hour << ":"
         << local_time->tm_min << ":"
         << local_time->tm_sec << "] "
         << "[" << levelToString(type) << "] [" << module << "] -"
         << msg << std::endl;
